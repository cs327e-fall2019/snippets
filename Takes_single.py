...
class NormalizeTakesFn(beam.DoFn):
  def process(self, element, class_pcoll):
    ...
    if (found_cno_match == False):
        ...
        cno_splits = cno.split('-') # assume bad record has hyphen
        cno = cno_splits[0]
...
def run():
  ...
  takes_sql = 'SELECT sid, cno, grade FROM workflow_college_modeled.Takes'
  class_sql = 'SELECT cno FROM workflow_college_modeled.Class'
  ...
  dataset_id = 'workflow_college_modeled'
  table_id = 'Takes_Beam'
