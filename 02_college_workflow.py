...
with models.DAG(
    ...
    split_student = DummyOperator(
            task_id='split_student',
            trigger_rule='all_done')
    
    student_beam = BashOperator(
            task_id='student_beam',
            bash_command='python /home/jupyter/airflow/dags/Student_single.py')
    
    student_dataflow = BashOperator(
            task_id='student_dataflow',
            bash_command='python /home/jupyter/airflow/dags/Student_cluster.py')
    
    takes_beam = BashOperator(
            task_id='student_beam',
            bash_command='python /home/jupyter/airflow/dags/Takes_single.py')
        
    create_staging_dataset >> create_modeled_dataset >> split
    split >> load_new_student >> load_current_student >> create_student >> create_takes >> split_student
    split >> load_class >> create_class >> create_teacher >> create_teaches
    split_student >> student_beam >> student_dataflow 
    split_student >> takes_beam